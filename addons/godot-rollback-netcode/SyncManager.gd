		...
		if node.is_inside_tree() and not node.is_queued_for_deletion():
			if node.has_method('_network_preprocess'):
				...
				if node is network_player_v1:
					player_input = input_frame.get_player_input(node.id_of_network_authority)
				else:
					player_input = input_frame.get_player_input(node.get_multiplayer_authority())
...
func _predict_missing_input(input_frame: InputBufferFrame, previous_frame: InputBufferFrame) -> InputBufferFrame:
	if not input_frame.is_complete(_player_peers):
		...
		for node in nodes:
			var node_master: int
			if node is network_player_v1:
				node_master = node.id_of_network_authority
			else:
				node_master = node.get_multiplayer_authority()
...
func get_latest_input_for_node(node: Node) -> Dictionary:
	if node is network_player_v1:
		return get_latest_input_from_peer_for_path(node.id_of_network_authority, str(node.get_path()))
	else:
...
func get_current_input_for_node(node: Node) -> Dictionary:
	if node is network_player_v1:
		return get_current_input_from_peer_for_path(node.id_of_network_authority, str(node.get_path()))
	else:
...
func _send_input_messages_to_peer(peer_id: int) -> void:
	...
	for input in _get_input_messages_from_send_queue_for_peer(peer):
		...
		var bytes = _message_serializer.serialize_input_message(peer_id, msg)
...
func _on_received_input_tick(peer_id: int, serialized_msg: PackedByteArray) -> void:
	...
	var msg = _message_serializer.unserialize_input_message(serialized_msg)
